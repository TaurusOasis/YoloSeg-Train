...
**07-03 修复（F11）**：改用 `RETR_CCOMP` 保留孔洞边界，同一实例的全部轮廓（断裂碎片 + 孔洞）经 `merge_multi_segment` 细桥合并为**每实例一行标签**——原 `RETR_EXTERNAL` + 每轮廓一行的做法会把孔洞填实、把遮挡断裂实例拆成多个实例（val 集抽样 2554 实例中 398 个受影响，15.6%）。修复后带孔实例还原 IoU 从 0.826 提升到 0.949；细桥在多碎片实例上引入轻微填充伪影（整体 mean IoU 0.938 与旧法持平），实例数语义正确性收益远大于此。**标签需重建后生效**（P2-1 重跑转换）。
...
### 3.1 ⚠️ 核心发现：在 COCONut val 标尺上，student 已追平 teacher（2026-07-03 补测）
...
2. 但它明确说明：**在当前训练分布上，teacher 的软监督已无增量信息可挖**——继续在 COCONut 上加大蒸馏权重、加蒸馏 epoch 的边际收益趋零甚至可能为负（teacher 的 mask 风格与 COCONut GT 相互竞争）。改进重心应转向数据/增强/时长，蒸馏侧只保留必要的正则作用（见 §5 计划）。
3. teacher 与 student 在 COCO val 官方标尺上的对比仍缺失（student 是否保住了 COCO 风格的泛化，还是过拟合 COCONut 标注风格），是下一步必测项（P1-1）。
...
**F13（流程，中→部分解决）验证标尺不统一，蒸馏收益无法量化。**
...
**07-03 更新**：teacher 在 COCONut val 的基线已补测（§3.1），结论是 student 已追平/反超——蒸馏 gap 在该标尺闭合。仍缺：COCO val2017 标尺对照、无蒸馏对照组。
...
| 编号          | 问题                            | 严重度                 | 改动量           | 建议时机                                     |
| ------------- | ------------------------------- | ---------------------- | ---------------- | -------------------------------------------- |
| F13           | 统一验证标尺 + teacher 基线缺失 | **高**（影响所有结论） | 运行侧           | **立即**（C 跑完前就可测 B/teacher）         |
| F11           | 转换标签孔洞/断裂实例损耗       | 中                     | 脚本重跑数据     | 短期（下一轮数据前）                         |
| F6            | 无 response 级蒸馏              | 中                     | distill_model.py | 短期消融                                     |
...
| F11           | 转换标签孔洞/断裂损耗           | 中                     | 脚本             | **✅ 代码已修（07-03），标签待重建（P2-1）** |
...
> 前提变化：阶段 C 已完成，且 §3.1 证明在 COCONut val 标尺上 teacher 已被追平——**"继续加大蒸馏"不再是主增长轴**，计划重心调整为：先补齐评测结论（P1）→ 数据与训练配方拿主要收益（P2）→ 蒸馏侧只做低成本消融与保正则（P3）→ 工程还债（P4）。

### P1 评测收口（不动训练代码，1 天内）

| #    | 任务                                        | 做法                                                                                                                                          | 产出/验收                                                                                                                                                   |
| ---- | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-1 | **COCO val2017 官方标尺对照**（F13 剩余项） | 用官方 COCO val 实例标注（转 YOLO seg 或走 `save_json=True` + pycocotools）评测：官方 yolo26s-seg / teacher / B best / C best 四个模型        | 一张 4×2 表（box/mask mAP50-95）。回答两个问题：① C 相对官方 yolo26s-seg 的差距（预期 C 略低，因标注风格偏移）；② C 是否因拟合 COCONut 风格而损失 COCO 泛化 |
...
| #    | 任务                                                                      | 做法                                                                                                                                                                                                          | 预期/验收                                                                                    |
| ---- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| P2-1 | **标签质量修复**（F11/F12）                                               | 转换代码已修（07-03，见 §4.7）；剩余动作：重跑 `build_coconut_yolo_seg.py --prepare-data --overwrite-data` 重建 COCONut-B 标签（建议输出到新 out-root 保留旧版对照）；iscrowd ignore 机制暂缓（需框架级支持） | 转换统计对比（实例数应下降、平均面积变化）；短程对照训练（30 epoch）mask AP 不降即可全量采用 |
| P2-2 | **配方升级重训**（替代原"续训"方案：lr 已衰减到 0.0006，resume 收益有限） | 从 C best.pt（或 B best.pt）起：`epochs=200, cos_lr=True, close_mosaic=20, copy_paste=0.4, mixup=0.1, multi_scale=0.25`，蒸馏保留但 `dis` 减半至 1.5（§3.1 结论：teacher 增量信息有限，保正则即可）           | 目标：COCONut val mask mAP50-95 ≥ 0.36（当前 0.341）。曲线出现平台才算训练充分               |
...
| P3-3 | response KD（F6）       | head cls logits 温度化 KL（`teacher_class_indices` 对齐通道），`dis_cls=1.0` 起步 | 鉴于 §3.1，预期收益有限；仅在 P1-1 显示 student 的 COCO 泛化明显弱于 teacher 时才值得投入 |
| P3-4 | 无蒸馏对照              | P2-2 同配方 `distill_model=None`                                                  | 量化蒸馏在"student 已追平 teacher"局面下的真实剩余价值，决定长期保留与否                  |
...
| P4-3 | 解耦重构（F1/F2/F3，安排在下次 rebase 上游前） | warmup 注入改 `on_train_batch_start` 回调；teacher 冻结/EMA 剥离/resume 重建收进 `DistillationModel` 接口；Detect/Segment 加显式 raw 输出接口替代 training-flag hack |
