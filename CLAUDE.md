...
- `default.yaml` — the single source of truth for _all_ training/val/predict/export settings and hyperparameters. Parsed by `cfg.get_cfg` into an `IterableSimpleNamespace`. CLI args and `model.train(...)` kwargs merge on top of this. When adding a setting, add it here with an inline `# (type) description` comment.
